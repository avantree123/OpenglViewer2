...
public:
    ...
    float l, r, b, t, d;
    int nx, ny;
    int samplesPerPixel = 64; // Q3 requirement: 64 samples per pixel
    ...
    std::vector<Ray> getRays(int ix, int iy) const {
        std::vector<Ray> rays;
        int sqrtSamples = 8; // 8x8 grid for 64 samples
        float step = 1.0f / sqrtSamples;
        for (int sy = 0; sy < sqrtSamples; ++sy) {
            for (int sx = 0; sx < sqrtSamples; ++sx) {
                float u_s = l + (r - l) * (ix + sx * step + step * 0.5f) / nx;
                float v_s = b + (t - b) * (iy + sy * step + step * 0.5f) / ny;
                vec3 direction = normalize(u_s * u + v_s * v - d * w);
                rays.push_back({ eye, direction });
            }
        }
        return rays;
    }
...
// Rendering Function (Updated for Q1 + Q2 + Q3)
void render(const Scene& scene, const Camera& camera) {
    ...
    for (int j = 0; j < ny; ++j) {
        for (int i = 0; i < nx; ++i) {
            vec3 color(0.0f);
            std::vector<Ray> rays = camera.getRays(i, j); // Q3: 64 rays per pixel
            for (const auto& ray : rays) {
                Intersection inter = scene.trace(ray, 0.0f, std::numeric_limits<float>::max());
                color += computePhong(inter, camera.eye, scene); // Q1
            }
            color /= float(camera.samplesPerPixel); // Q3: Box filter averaging
            color = applyGamma(color); // Q2: Gamma correction
